...
static void oom_exit(const char *msg){
    ...
    ed->size = 0;
    ed->capacity = initial_cpcty;
    ed->lines = malloc(ed->capacity * sizeof(char *));
    if (!ed->lines) oom_exit("malloc");
...
void shrinkToFit(Editor *ed){
    ...
    if (ed->size == 0) {
        free(ed->lines);
        ed->lines = malloc(initial_cpcty * sizeof(char *));
        if (!ed->lines) oom_exit("malloc");
        for (size_t i = 0; i < initial_cpcty; ++i) ed->lines[i] = NULL;
        ed->capacity = initial_cpcty;
        return;
    }
...
char *readInputLine(const char *prompt){
    ...
    char *buf = NULL;
    size_t cap = 0;
    size_t nread = getline(&buf, &cap, stdin);
    if (nread == -1) {
        free(buf);
        return NULL;
    }
